  ...
  IngredientMapper.Domain == [IngredientModel]
{
  ...
  private let ingredientMapper: IngredientMapper

  public init(ingredientMapper: IngredientMapper) {
    self.ingredientMapper = ingredientMapper
  }

  public func transformResponseToEntity(request: String?, response: MealResponse) -> MealEntity {
    let ingredients = self.ingredientMapper.transformResponseToEntity(request: request, response: response)
  ...
  public func transformEntityToDomain(entity: MealEntity) -> MealModel {
    let ingredients =  self.ingredientMapper.transformEntityToDomain(entity: entity.ingredients)
